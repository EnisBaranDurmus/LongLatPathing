...
R = 6371000
class locPoint:
    def __init__(self, bearing, distance, lon, lat) -> None:
        self.bearing = bearing
        self.distance = distance
        self.lon = math.radians(lon)
        self.lat = math.radians(lat)
        self.angularD = distance / R
    def distanceBearing(self):
        lat2 = math.asin(math.sin(self.lat) * math.cos(self.angularD) + 
        math.cos(self.lat) * math.sin(self.angularD) * math.cos(self.bearing))

        lon2 = self.lon + math.atan2(math.sin(self.bearing) * math.sin(self.angularD)
        * math.cos(self.lat), math.cos(self.angularD) - math.sin(self.lat) * math.sin(lat2))

        dlon = self.lon - lon2
        y = math.sin(dlon) * math.cos(self.lat)
        x = math.cos(lat2) * math.sin(self.lat) - math.sin(lat2) * math.cos(self.lat) * math.cos(dlon)
        z = math.atan2(y,x)
        finalBearing = toDegree(math.degrees(z + math.pi))
        
        result = secondLocPoint(round(math.degrees(lat2), 8),  round(math.degrees(lon2), 8), finalBearing)
        return result
...
def distanceBearing():
    ...
    result = secondLocPoint(round(math.degrees(lat2), 7),  round(math.degrees(lon2), 7), finalBearing)
    return result
...
object1 = locPoint(0,2,69,31)
object = object1.distanceBearing()
